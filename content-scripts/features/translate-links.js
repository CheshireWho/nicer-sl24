...
const initTranslateLinkCreation = () => {
    ...
    const getTranslateLinkElem = () => {
        ...
        const TranslateLinkElem = document.createElement('a');
        TranslateLinkElem.classList.add(translateLinkClass);
        TranslateLinkElem.target = '_blank';
        TranslateLinkElem.appendChild(SvgWrap.children[0]);

        return TranslateLinkElem;
    };
    ...
    const translateClickHandler = (event) => {
        ...
        if (Link && textToTranslate) {
            Link.href = `https://translate.google.com/?sl=${translateSourceLanguage}&tl=${translateTargetLanguage}&op=translate&text=${encodeURI(textToTranslate)}`;
        }
    };

    // Add a translate link to each selector match.
    ...
    const appendTranslateLink = (selector, className) => {
        ...
        elemsToGetLink.forEach((elem) => {
            const TranslateLinkElem = getTranslateLinkElem();
            TranslateLinkElem.classList.add(className);
            elem.appendChild(TranslateLinkElem);

            TranslateLinkElem.addEventListener('click', translateClickHandler);
        });
    };
